if ActiveRecord::Base.connection.table_exists? 'rpush_notifications'
  ...
  # Setup
  unless ENV["APN_APP_NAME"].nil?
    Rpush::Apns::App.find_or_create_by(name: ENV["APN_APP_NAME"]) do |app|
      app.name = ENV["APN_APP_NAME"]
      app.certificate = File.read(ENV["CERTIFICATE_PATH"])
      app.environment = ENV["APN_ENV"] # APNs environment.
      app.password = ENV["CERITIFICATE_PW"]
      app.connections = ENV["CONNECTIONS"] || 1
    end
  end
end
